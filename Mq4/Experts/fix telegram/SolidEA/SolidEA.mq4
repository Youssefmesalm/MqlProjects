...
input closeStrategy close_Strategy = single;
...
void OnTick() {
  ...
  for(int i = 0; i < size; i++) {
    ...
    int mainSignal = 0;
    if(indikator1 != off) {
      MasterSignal[i] = GetIndicatorsSignal(indikator1, timeframe1, comment1, shift1, Symbols[i]);
    }
    if(indikator2 != off) {
      Signal2[i] = GetIndicatorsSignal(indikator2, timeframe2, comment2, shift2, Symbols[i]);
    }
    if(indikator3 != off) {
      Signal3[i] = GetIndicatorsSignal(indikator3, timeframe3, comment3, shift3, Symbols[i]);
    }
    if(indikator4 != off) {
      Signal4[i] = GetIndicatorsSignal(indikator4, timeframe4, comment4, shift4, Symbols[i]);
    }
    if(indikator1x != off) {
      exit1[i] = GetIndicatorsSignal(indikator1x, timeframe1x, comment, 0, Symbols[i]);
    }
    if(indikator2x != off) {
      exit2[i] = GetIndicatorsSignal(indikator2x, timeframe2x, comment4, 0, Symbols[i]);
    }
    // ======================================== Single Strategy ===================================
    if(Strategy == single) {
      // =====================================Buy =====================================
